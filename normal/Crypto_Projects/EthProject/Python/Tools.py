...
def range_check(r_time):
    ...
    t = time()
    h_time = datetime.fromtimestamp(r_time * (r2 - r1) + time(), pytz.timezone("Europe/Moscow")).strftime("%A, %B %d, %Y %H:%M:%S")
    ...
    for x in range(r1, r2):
        dec_to_address(x)
    print(f"Time Remaining: {h_time}")
    print(datetime.fromtimestamp(time(), pytz.timezone("Europe/Moscow")).strftime("%A, %B %d, %Y %H:%M:%S"))
    t2 = time()
    print(t2 - t)
