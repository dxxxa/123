# Автоматический генератор биткойн-кошельков, который генерирует случайные адреса кошельков
# с соответствующим закрытым ключом и проверяет их баланс в режиме реального времени.
...
def process(data, balance):
    ...
    if (balance == 0.00000000):
        ...
        print(Back.RED + Fore.BLACK + (str(address)) + " : " + str(balance) + " (BTC)")
        #print(Back.RED + Fore.BLACK + "{:<34}".format(str(address)) + " : " + str(private_key) + " : " + str(private_key_to_WIF(private_key)) + " : " + str(balance))
    if (balance > 0.00000000):
        print(Back.GREEN + Fore.BLACK + (str(address)) + " : " + str(balance) + " (BTC)")
        #print(Back.GREEN + Fore.BLACK + "{:<34}".format(str(address)) + " : " + str(balance) + " (BTC)")
