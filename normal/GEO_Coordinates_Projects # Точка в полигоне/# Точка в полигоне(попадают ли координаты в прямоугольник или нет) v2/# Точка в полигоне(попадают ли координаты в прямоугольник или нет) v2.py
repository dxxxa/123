...
# https://github.com/b0006/Point-in-polygon?ysclid=l6evgizogd791305229
...
def IsPointInsidePolygon(polygon, number_of_vertices, x, y):
    ...
    for n in range(number_of_vertices):
        ...
        while flag == False:
            ...
            # первая площадь
            S1 = math.fabs(polygon.x[i1] * (polygon.y[i2] - y) + polygon.x[i2] * (y - polygon.y[i1]) + x * (
                        polygon.y[i1] - polygon.y[i2]))
            # вторая площадь
            S2 = math.fabs(polygon.x[n] * (polygon.y[i2] - y) + polygon.x[i2] * (y - polygon.y[n]) + x * (
                        polygon.y[n] - polygon.y[i2]))
            # третья площадь
            S3 = math.fabs(polygon.x[i1] * (polygon.y[n] - y) + polygon.x[n] * (y - polygon.y[i1]) + x * (
