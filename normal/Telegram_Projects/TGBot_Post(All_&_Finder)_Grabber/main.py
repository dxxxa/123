# https://github.com/dasunpamod/Atg-clone
...
# +79228969421
api_id = 22502374
api_hash = "5a8751daf28e69e1c1a376d9a3ebfa0d"

#api_id = 28297361
#api_hash = "e0eed1ec03e7de3c444d73f3aef9ce68"

client = TelegramClient('TrashStash', api_id, api_hash)
...
async def main():
    ...
    async for message in client.iter_messages(target_entity, reverse=True, min_id=start_from, wait_time=1):
        try:
            # По умолчанию отправляет копию сообщения (переслать без кавычек)
            # Если нужно переслать с цитатой, просто замените client.send_message(...) на client.forward_messages(...)
            ...
            print(datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
                  'FLOAD ERROR DETECT! WE MUST WAIT', e.seconds, 'seconds until next forward')
            print('You can exit with Ctrl-c, your last message tried to forward is', message.id - 1)

            time.sleep(e.seconds + 10)

            # After e.seconds time (and more) of waiting we retry to forward the message ()
            # In case of another fail (example because we can't forward this kind of message) we don't care and continue
            # Another flood exception should not happen, can't do anything with exception that are not a flood exception
            try:
                await client.forward_messages(destination_entity, message)
                forwarded_n = forwarded_n + 1
                print('Message', message.id, 'forwarded! Total forwarded:', forwarded_n, 'Total errors:', error_n)
